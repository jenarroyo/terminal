...
{   //know if it is a file or directory
    ...
    {   //source param is not a file. it is a directory
        $success = -2; 
    }
}
else 
...
ChromePhp::log("Success stat:  $success");

if($success===true)
{
    $destSlashIndex = strpos($destDirFile, "/");
    if($destSlashIndex===0) 
    {   //remove the slash if it is root.
        $destDirFile = substr($destDirFile, 1); 
    }
    ...
    if(is_dir($directory . substr($destDirFile, 0, strrpos($destDirFile,"/"))))
    {
        ...
        { //check if the input is a directory itself (e.g. nested/f.1)
